							 		...
							 		<ul id="quantity_<?php echo $productIdName1->product_id; ?>">
							 			...
							 			foreach($productVariation as $productVariations) {
							 				...
							 				$arrayVariationId=explode(',',$productVariations['variation_ids']);
								   			...
								   			<li>
												<div>
													<input type="checkbox" name="variationCheckbox[]" class="variationCheckbox" value="<?php echo $productVariations['sku']; ?>" attrProductId="<?php echo $productIdName1->product_id; ?>" attrVariationRelation="<?php echo $productVariations['id']; ?>" style="float:left;width:400px;"/>
												</div>	
												<div>
													<input type="checkbox" name="brcdSrCheckBox[]" 	id="brcdSrCheckBox_<?php echo $productVariations['sku']; ?>" onchange="$('#quantity_keypress_<?php echo $productVariations['sku']; ?>').change();" class="brcdSrCheckBox" value="<?php //echo $productVariations['sku']; ?>" attrProductId="<?php echo $productIdName1->product_id; ?>" attrVariationRelation="<?php echo $productVariations['id']; ?>" style="float:right;"/>
												</div>	
												<label style="float:right;">Barcode</label>
								   				...
								   				<ul>
								   					<li>
														...
														<input class="quantity_input quantity_keypress" onchange="GetAddBrcdSr(this,'<?php echo $productVariations['sku']; ?>');" id="quantity_keypress_<?php echo $productVariations['sku']; ?>" type="text"  name="quantity[<?php echo $productVariations['sku']; ?>]" value="" />  <span class="add_sr_no" p_id = "quantity_keypress_<?php echo $productVariations['sku']; ?>">Add Sr No.</span>
														</span>
...
<script>
   ...
   /*	$('.batches').change(function() {
		...
		for (var i = 1; i <= qt; i++) {
			...
			$(this).before('<input type="text" class="sr_no_dynamic" style ="margin-top:20px"  name="brcd_sr[]" >');
		}
...
function GetAddBrcdSr(ls_v,ls_sku)
{
	var ls_cls = "sr_no_dynamic"+ls_sku;
	var qt = $(ls_v).val();
	var is_checked = $('#brcdSrCheckBox'+ls_sku).is(':checked');
	$('.'+ls_cls).remove();
	//alert(is_checked);
	//alert(qt);
	//if(is_checked == true)
	{
		for (var i = 1; i <= qt; i++) 
			{
				$(ls_v).after('<input type="text" class="'+ls_cls+'" style ="margin-top:20px"  name="brcd_sr[]" >');
			}
	}		
}   
</script>
