...
<div class="page-content">
   ...
   <div class="row-fluid">
      <div class="span12">
         ...
         <div class="row-fluid">
            <div class="span12">
               <div class="table-header tableThemeColor"> Results for "Registered Vendors" </div>
               <table id="myTable" class="table table-striped table-bordered table-hover">
                  ...
                  <tbody>
                     <?php 
                        ...
                        $permission_array = checkPermissionByUserRole($uinfo['user_role'],16);
                        foreach($vendors as $vendor){
                       // print_r($vendor);
						?>
                     <?php //if($uinfo['user_level']<$vendor->user_level) {
					 //if($vendor->user_role!=5 || $uinfo['user_level']==1){
					 ?>
                     <tr>
                        <td class="center"><?php echo $ctr;?></td>
                        <td><?php echo $vendor->f_name." ".$vendor->l_name." ";?></td>
                        <td><?php echo $vendor->email;?></td>
                        <td><?php echo $vendor->username;?></td>
                        <td><?php echo $vendor->city;?></td>
                        <td><?php echo $vendor->address; ?></td>
						<td><?php echo $vendor->zipcode; ?></td>
						<td><?php echo $vendor->contact_number; ?></td>
						<td><?php echo $vendor->firm_name; ?></td>
						<td><?php echo $vendor->tin_number; ?></td>
						<!--<td><?php echo $vendor->account_number; ?></td>
						<td><?php echo $vendor->ifsc_code; ?></td>
						<td><?php echo $vendor->account_name; ?></td>
						<td><?php echo $vendor->bank_name; ?></td>-->
						<td><a href="javascript:void(0);" id="id-btn-dialog_<?php echo $vendor->vendor_id;?>" class="btn btn-purple btn-sm vendor_bank_link">Bank Details</a></td>
						<?php $vendor_bank_details = getBankDetailsById($vendor->vendor_id);?>
						<div id="dialog-message_<?php echo $vendor->vendor_id;?>" class="hide">
											<?php foreach($vendor_bank_details as $single){?>
											<p>
						...
						<td>    
                           <label>
                           <input id="vendor_status_switch_<?php echo $vendor->vendor_id;?>" name="vendor_status_switch_<?php echo $vendor->vendor_id;?>" class="ace-switch ace-switch-3" type="checkbox" <?php if($vendor->vendor_status==1){echo "checked=checked";} ?> />
                           <span class="lbl"></span>												</label>
                           <?php if($vendor->vendor_status==1){echo '<span style=" visibility:hidden">on</span>';} ?>
                           <?php if($vendor->vendor_status==0){echo '<span style=" visibility:hidden">off</span>';} ?>
                        </td>
                        ...
                        <td>
                           <div class="hidden-phone visible-desktop btn-group">
                              <?php if($permission_array[0]['edit']=='1') { ?>
                              <a href="<?php echo site_url()."webadmin/managevendor/editVendor/".$vendor->vendor_id;?>" class="tooltip-info" data-rel="tooltip" title="Edit">
                              <button class="btn btn-mini btn-info"> <i class="icon-edit bigger-120"></i> </button>
                              </a>
                              <?php }?>
                              <?php if($permission_array[0]['delete']=='1') { ?>
                              <a id="delBtn_<?php echo $vendor->vendor_id;?>" class="delBtn tooltip-info" href="<?php echo site_url()."webadmin/managevendor/deleteVendor/".$vendor->vendor_id;?>" data-rel="tooltip" title="Delete">
                        ...
                        <?php }else{echo "<td>Access Denied</td>";}?>
						<td> 
							<a href="<?php echo site_url()."webadmin/managevendor/changePassword/".$vendor->vendor_id;?>" class="tooltip-info" data-rel="tooltip" title="Edit">
...
<script>
   $(function(){
   		...
   		$( ".ace-switch-3" ).change(function() {
   			...
   			vendor_id=$(this).attr('id').split('vendor_status_switch_');
   			var url="<?php echo site_url();?>webadmin/managevendor/changeVendorStatus";
   			$.ajax({
   			url: url,
   			type:'GET',
   			data:"vendor_status="+change_status_to+"&vendor_id="+vendor_id[1],
...
$( ".vendor_bank_link").click(function(){
