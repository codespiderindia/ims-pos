...
Not to print!
	...
	<div class="widget-body">
		...
		<div class="a4-container">
			<div class="a-head"><?php echo isset($sale_store_info['comp_name']) ? $sale_store_info['comp_name'] : (isset($companyDetails) ? $companyDetails->comp_name : ''); ?></div>
	...
	<div class="a-table">
	 <table class="a-main a-main1">
	   ...
	   <tr>
	     <td colspan="4">
		   <p>Company Name : <b><?php echo isset($sale_store_info['comp_name']) ? $sale_store_info['comp_name'] : (isset($companyDetails) ? $companyDetails->comp_name : ''); ?></b></p>
		   <p>Company Address : <b><?php echo isset($sale_store_info['comp_address']) ? $sale_store_info['comp_address'] : (isset($companyDetails) ? $companyDetails->comp_address : ''); ?></b></p>
		   <p>Store Name: <?php echo $sale_store_info['store_name'];?></p>
		   <!-- <p>GST Rel 6.0 Preview</p> -->
		   <!-- <p>Madurai</p> -->
		   <p>GSTIN/UIN : <?php echo (isset($companyFirmInfo->firm_teen_num)) ? $companyFirmInfo->firm_teen_num : ""; ?></p>
		 </td>
		 <td colspan="4">
		   <table class="a-main-in">
		     <tr>
				<td>Invoice No. <?php echo $sale_detail['invoice_number'];?></td>
	   ...
	   <tr>
	     <td colspan="5">
	     <p>Sale Customer Info.</p>
	     <?php
	     	if(isset($sale_customer) && !empty($sale_customer)){
	     		?>
					<p><b><?php echo $sale_customer['fname']." ".$sale_customer['lname'];?></b></p>
					
					<p>GSTIN/UIN : 33AAAAA1234A1Z5</p>
					<p>Place of Supply : <?php echo getStateNameByID($sale_customer['state_id']);?></p>
	     		<?php
	     	} else{
	     		?>
	     		NIL
	 ...
	 <table class="a-main a-main2 a-main-two">
	   ...
	   <?php if(isset($sale_items) && !empty($sale_items)){ ?>
		<?php 
			...
			foreach($sale_items as $item){
				...
				/*item subtotal on behalf gst inc or exc*/
				if($item['gst_inc'] == 0)
		 		{
					$item_subtotal = $item['item_subtotal'];
		 		}
		 		else{
		 			$item_subtotal = $item['item_subtotal'] - ($item['cgst_amt']+$item['sgst_amt']+$item['igst_amt']);
		 		}
				/*item subtotal on behalf gst inc or exc*/
		?>
	   <tr>
	     <td><?php echo $ctr;?></td>
		 <td><?php echo $item['product_ID'].'-'.$item['product_detail'];?></td>
		 <td>12345678</td>
		 <td><?php echo $item['tax_per'];?> %</td>
		 <td><?php echo $item['quantity'];?></td>
		 <td><?php echo $item['item_cost_price'];?></td>
		 <td><?php echo $item['product_unit'];?></td>
		 <!-- <td><?php echo $item['item_subtotal'];?></td> -->
		 <td><?php echo $item_subtotal;?>
		 </td>
	   ...
	   <tr>
		 ...
		 <td></td>
		 <td></td>
		 <!--<td class=""><?php echo  $sale_detail['sub_total'];?>.00</td>-->
		 <td class=""><?php echo  $sale_detail['sub_total'];?></td>
	   </tr>
			<?php  } ?>
		<tr>
	     <td></td>
		 ...
		 <td></td>
		 <td></td>
		 <td><?php echo $sale_detail['discount_per'];?>.%</td>
		 <td class=""><?php echo $sale_detail['discount_amt'];?></td>
	   </tr>
	   ...
	   <tr class="total">
		 ...
		 <td></td>
		 <td></td>
		 <td><?php echo  $sale_detail['total_items'];?> nos</td>
		 <td></td>
		 <td></td>
		 <td><b>Rs. <?php echo  $sale_detail['total_new'];?></b></td>
	   </tr>
	 ...
	 <table class="a-main a-main2">
	 	...
	 	<tr class="payment_content" style="border-bottom: 1px solid #ccc">
		 	...
		 	foreach($sale_items as $item){
		 		 /* Get Offer Details */
					...
					?>
						 <tr>
					 		 	<td><?php echo $ctr; ?></td>
					 		 	<td><?php echo $item['product_detail'].' ('.$item['product_ID'].')'; ?></td>
					 		 	<td><?php echo $item['item_subtotal']; ?></td>
	 	 ...
	 	 <tr class="total">
	 	 	<td></td>
	 		<td>Total</td>
	 		<td><?php echo  $sale_detail['total_new'];?></td>
	 		<td></td>
	 		<td></td>
	 ...
	 <!--<table class="a-main a-main2">
	 	...
	 	<tr class="payment_content" style="border-bottom: 1px solid #ccc">
	 	 ...
	 	 <?php if(isset($sale_items) && !empty($sale_items)){
	 	 $ctr=1;
	 	 $totalTax=$totalOffer=$amt=0;$offerVal='';
	 	 foreach($sale_items as $item){
	 ...
	 <table class="a-main a-main2">
	 	<tr>
	 		<td colspan="3" class="a-main-title a-main-title22" style="height:32px;"><p>Payment Details</p></td>
	 	</tr>
	 	<tr class="payment_content" style="border-bottom: 1px solid #ccc">
	 		<td class="pay_td">S.No.</td>
	 		<td class="pay_td">Payment Method</td>
	 		<td class="pay_td">Amount</td>
	 	</tr>
	 	<?php if(isset($sale_payments) && !empty($sale_payments)) {
	 			$i = 1;
	 	 		foreach($sale_payments as $sale_paymentss) { ?>
		 	<tr class="payment_content">
		 		<td class="pay_td"><?php echo $i; ?></td>
		 		<td class="pay_td"><?php echo ucfirst($sale_paymentss['payment_method']); ?></td>
		 		<td class="pay_td"><?php echo $sale_paymentss['payment_amount'] ?></td>
		 	</tr>
	 	<?php $i++; } ?>
	 	<tr>
	 		<td></td>
	 		<td></td>
	 		<td class="ttl"><?php echo  $sale_detail['total_new'];?></td>
	 	</tr>
	 	<?php } ?>
	 </table>
	  &nbsp;
	 ...
	 <div class="mn-text mn-text1">
	   ...
	   <p><b><?php echo ucwords(@getIndianCurrency($sale_detail['total_new']));?></b> Only</p>
	 </div>
	 ...
	 <tr>
	 	...
	 	<td>Amount</td>
	 	   ...
	 	   <?php if(isset($sale_items_cp) && !empty($sale_items_cp)){ ?>
		<?php 
		$ctr=1;
		$total_taxable = 0;
		foreach($sale_items_cp as $item_cp){
			//$cgst_tot_amt= $cgst_tot_amt+$item['cgst_amt'];
			//$sgst_tot_amt= $sgst_tot_amt+$item['sgst_amt'];
			//$igst_tot_amt= $igst_tot_amt+$item['igst_amt'];

			if($item_cp['cgst_amt'] != 0 && $item_cp['sgst_amt'] != 0) {
	 			$breakdownTotal = $item_cp['item_subtotal'] - ($item_cp['cgst_amt'] + $item_cp['sgst_amt']);
	 		} else {
	 			$breakdownTotal = $item_cp['item_subtotal'] - $item_cp['igst_amt'];
	 		}

	 		if($item_cp['gst_inc'] == 0)
	 		{
	 			$breakdownFinalTotal = $breakdownTotal;
	 		}
	 		else{
	 			$breakdownFinalTotal = $breakdownTotal - ( $breakdownTotal * ($item_cp['tax_per']/100) );
	 		}
	 		$total_taxable = $total_taxable + $breakdownFinalTotal;
			?>
	 <tr>
	 	<td colspan="2"><?php echo $item_cp['product_detail'].' ('.$item_cp['product_ID'].')'; ?></td>
	 	<td colspan="2"><?php echo number_format($breakdownFinalTotal,2);?></td>
	 ...
	 <tr class="total">
	 	...
	 	<?php
	 	 ...
	 	 ?>
	 	<td colspan="2"><?php echo number_format($total_taxable,2);//echo $sale_detail['sub_total']; ?></td>
	 	<td></td>
	 ...
	 <div class="mn-text mn-text2">
		...
		<div>
			<p style="font-weight: 600;">Remark : </p>
	   		<p style="font-size: 14px;"><?php echo $sale_detail['remark']; ?></p>
		</div>
	 </div>
	 ...
	 <table class="a-main a-main4">
	   ...
	   <tr>
		 ...
		 <td class="a-main4-right">
		  <table class="a-main-in">
		     <!--<tr>
			   <td class="CBD" colspan="2">Company's Bank Details</td>
			 </tr>
			 <tr>
			   <td>Bank Name :</td>
			   <td></td>
	...
	<div>
		<label class="invBarcode_label">Invoice Barcode</label>
		<img src="<?php echo base_url(); ?>barcode/sample-gd.php?code=<?php echo base64_encode($sale_detail['invoice_number']); ?>" width="500px" height="100px" />
	</div>
...
$('#inv_print_opt').change(function(){
	...
	var saleID=<?php echo $sale_ID?>;
	
					var url="<?php echo base_url();?>sales/managesales/change_invoice_format/";
					$.ajax({
					url: url,
					type:'POST',
					data:"inv_opt="+inv_opt+"&saleID="+saleID,
...
/*function printDiv() 
{
  var divToPrint=document.getElementByClassName('a4-container');
