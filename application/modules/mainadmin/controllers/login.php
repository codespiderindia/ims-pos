		...
		{		
			...
			$uInfo=$this->session->userdata('mainadmin_session_info');
				...
				// ensure already signed in 
					if($uInfo['logged_in']==TRUE){redirect('mainadmin/dashboard');}
				}
	...
	public function forgotpassword(){
		...
		if($this->form_validation->run('forgotpassword')){
			...
			if($flg){
				$data['success']=1;
				echo '
<div style="text-shadow:none; color:#000;background-color:none;"> An Email has been sent on your registered email address, Please check your Inbox for more detail. </div>
';
