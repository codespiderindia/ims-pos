...
<script type="text/javascript">
	$(document).ready(function(){
		/*$('.jscolor').on('click', function() {
			...
			$.ajax({
				type:'POST',
				url:"<?php echo site_url();?>webadmin/managetheme/themeColor",
				data:'colorCode='+colorCode+'&themeName='+themeName,
				success:function(html){
					//$('#state').html(html);
					//$('#city').html('<option value="">Select state first</option>');
					$.ajax({
						type:'POST',
						url:"<?php echo site_url();?>webadmin/managetheme/getThemeColor",
						success:function(result){
							$(".navbar-inner, .welcomeThemeColor, .buttonThemeColor, .nav-list>li>a:hover:before").attr('style', 'background:'+result+' !important; border-color:' +result+ '!important');

							var avoided = result.replace('#','');
							$(".jscolor").val(avoided); 
						}
					}); 
				}
	...
	/*$(window).load(function() {
		$.ajax({
			...
			url:"<?php echo site_url();?>webadmin/managetheme/getThemeColor",
			success:function(result){
				$(".navbar-inner").attr('style', 'background:'+result+'!important;');
				$(".nav-list>li>a:hover:before").attr('style', 'background-color:'+result+'!important;');  
				var avoided = result.replace('#','');
				$(".jscolor").val(avoided); 
				//$('#state').html(html);
				//$('#city').html('<option value="">Select state first</option>'); 
